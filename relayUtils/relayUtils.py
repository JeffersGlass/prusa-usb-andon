...
class relayBoard():
    def __init__(self, libpath='.'):
        ...
        self.blinkLock = threading.RLock()
    ...
    @classmethod
    def singleBoard(cls, libpath = '.'):
      r = relayBoard(libpath)
      r.loadLib()
      ids = r.enumDevs()
      if len(ids) > 0:
        r.openDevById(ids[0])
      else: raise RuntimeError("No relay board found")
      return r
    ...
    def blinkRelay(self, num, timing = 1):
        logging.debug("Before changing blink start, blinking{} = " + str(self.blinking))
        with self.blinkLock:
          self.blinking[num] = True
        logging.debug("After changing blink start, blinking{} = " + str(self.blinking))
        logging.debug("End of blinkRelay function")
    ...
    def __blink(self):
        while (self.blinkAlive):
          ...
          for i in range(1, self.numRelays+1):
            #logging.debug("Blink about to acquire lock")
            with self.blinkLock:
              #logging.debug:("Blink aquired lock")
